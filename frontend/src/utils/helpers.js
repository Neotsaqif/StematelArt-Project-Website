...
export const imgUrl = (photoId, w, h) => {
...
export const fmtPts = (n) => n.toLocaleString("id-ID");
export const lifetimeScore = (a) => a.likes * 12 + a.comments * 30 + Math.round(a.views * 0.6);
...
export function computeRows(items, containerW, targetH, gutter) {
...
// Toast Bus System
export const toastBus = { list: [], subs: new Set(), n: 0 };
export const emitToast = () => toastBus.subs.forEach(f => f([...toastBus.list]));

export function pushToast(type, title, opts) {
  const id = ++toastBus.n;
...
export const pwStrength = (v) => (v.length === 0 ? 0 : v.length < 8 ? 1 : /[^a-zA-Z]/.test(v) ? 3 : 2);
