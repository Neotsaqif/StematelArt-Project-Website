...
export function JustifiedGrid({ artworks, targetHeight = 240 }: JustifiedGridProps) {
  const app = useApp();
  ...
  const artMap = useMemo(() => new Map<number, Artwork>(artworks.map(a => [a.id, a])), [artworks]);
...
export function MobileGrid({ artworks, onArtworkClick }: MobileGridProps) {
  return (
